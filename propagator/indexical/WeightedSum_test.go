...
func sumweighted_ic_test(t *testing.T, varsMapping []*VarMappingWeighted,
	...
	ready_test(t, "WeightedSum_IC", ready, expready)

	if expready {
		for _, varMapping := range varsMapping {
			expDomain := core.CreateIvDomainFromIntArr(varMapping.expDomain)
			DomainEquals_test(t, "WeightedSum_IC",
				varMapping.intVar, expDomain)
		}
		sumexpDomain := core.CreateIvDomainFromIntArr(sum.expDomain)
		DomainEquals_test(t, "WeightedSum_IC", sum.intVar, sumexpDomain)
	}
}

func Test_WeightedSum_ICa(t *testing.T) {
	setup()
	defer teardown()
	log("WeightedSum_ICa: 1*X + 2*Y = Z, X:0..2, Y:0..2, Z:6")

	X := CreateVarMappingWeighted(1, []int{0, 1, 2}, []int{2})
	Y := CreateVarMappingWeighted(2, []int{0, 1, 2}, []int{2})
	sum := CreateVarMapping([]int{6}, []int{6})
	sumweighted_ic_test(t, []*VarMappingWeighted{X, Y}, sum, true)
}

func Test_WeightedSum_ICb(t *testing.T) {
	setup()
	defer teardown()
	log("WeightedSum_ICb: 1*X + 2*Y + 3*Q = Z, X:0..2, Y:0..2, Q:0..2, Z:12")

	X := CreateVarMappingWeighted(1, []int{0, 1, 2}, []int{2})
	Y := CreateVarMappingWeighted(2, []int{0, 1, 2}, []int{2})
	Q := CreateVarMappingWeighted(3, []int{0, 1, 2}, []int{2})
	sum := CreateVarMapping([]int{12}, []int{12})
	sumweighted_ic_test(t, []*VarMappingWeighted{X, Y, Q}, sum, true)
}

func Test_WeightedSum_ICc(t *testing.T) {
	setup()
	defer teardown()
	log("WeightedSum_ICc: 1*X + 2*Y + 3*Q = Z, X:0..2, Y:0..2, Q:0..2, Z:13")
	...
	sumweighted_ic_test(t, []*VarMappingWeighted{X, Y, Q}, sum, false)
}

func Test_WeightedSum_ICd(t *testing.T) {
	setup()
	defer teardown()
	log("WeightedSum_ICd: 1*A + 1*B = C, A:0..4, B:0..4, C:[5..10, 12..15]")
	...
	sumweighted_ic_test(t, []*VarMappingWeighted{A, B}, sum, true)
}

func Test_WeightedSum_ICe(t *testing.T) {
	setup()
	defer teardown()
	log("WeightedSum_ICe: 1*A + 1*B = C, A:0..4,9 , B:0..4,9 , C:[5..10, 12..15]")
...
func Test_WeightedSum_ICf(t *testing.T) {
	setup()
	defer teardown()
	log("WeightedSum_ICf: 1*A + 1*B = Z, A:0..4,11 , B:0..4,11 , C:[5..10, 12..15]")
