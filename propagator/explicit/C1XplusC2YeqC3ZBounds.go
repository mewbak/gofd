...
// C1XplusC2YeqC3ZBounds propagator for C1*X + C2*Y = C3*Z
...
// makeBoundConsistent checks if the constraint is bound consistent and
// makes it bound consistent if possible.
...
func CreateXplusYeqZBounds(x core.VarId, y core.VarId, z core.VarId) *C1XplusC2YeqC3ZBounds {
	if core.GetLogger().DoDebug() {
		core.GetLogger().Dln("CreateGXplusYeqZ-propagator")
	}
	prop := new(C1XplusC2YeqC3ZBounds)
	prop.x = x
	prop.y = y
...
func (this *C1XplusC2YeqC3ZBounds) String() string {
	return fmt.Sprintf("PROP_%d %d*%s+%d*%s=%d*%s",
		...
		this.c2, core.GetNameRegistry().GetName(this.z))
}
