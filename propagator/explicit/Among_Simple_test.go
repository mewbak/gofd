...
func among_simple_test(t *testing.T, xiinit [][]int, kinit []int,
	...
	log(y)
	ready_test(t, "Among_Simple", ready, expready)
	if expready {
		domainEquals_test(t, "Among_Simple", N, expn)
		for i := 0; i < len(expxi); i++ {
			domainEquals_test(t, "Among_Simple", Xi[i], expxi[i])
		}
	}
}
...
func Test_AmongSimplea(t *testing.T) {
	setup()
	defer teardown()
	log("Among_Simple: Xi:{{1},{2},{3}}, K:{1,2}, N:{2}")
	xi := make([][]int, 3)
	xi[0] = []int{1}
	xi[1] = []int{2}
	xi[2] = []int{3}
	...
	among_simple_test(t, xi, k, n, xi, n, true)
}

func Test_AmongSimpleb(t *testing.T) {
	setup()
	defer teardown()
	log("Among_Simple: Xi:{{2},{4},{5}}, K:{2,3,4}, N:{1}")
	xi := make([][]int, 3)
	xi[0] = []int{2}
	xi[1] = []int{4}
	xi[2] = []int{5}
	...
	among_simple_test(t, xi, k, n, xi, expn, false)
}

func Test_AmongSimplec(t *testing.T) {
	setup()
	defer teardown()
	log("Among_Simple: Xi:{{2,3},{4,5},{5,6}}, K:{2,3,4}, N:{1}")
	xi := make([][]int, 3)
	xi[0] = []int{2, 3}
	xi[1] = []int{4, 5}
	xi[2] = []int{5, 6}
...
func Test_AmongSimpled(t *testing.T) {
	setup()
	defer teardown()
	log("Among_Simple: Xi:{{2,3},{4,5},{5,6}}, K:{2,3,4}, N:{4}")
	xi := make([][]int, 3)
	xi[0] = []int{2, 3}
	xi[1] = []int{4, 5}
	xi[2] = []int{5, 6}
