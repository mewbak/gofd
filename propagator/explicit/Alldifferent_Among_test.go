...
func alldifferent_Among_test(t *testing.T, xiinit [][]int, expxi [][]int,
	expready bool) {
	...
	p := CreateAlldifferent_Among(Xi, store)
...
func Test_AlldifferentAmonga(t *testing.T) {
	setup()
	defer teardown()
	log("Alldifferent_Among: Xi:{{1,2,3},{1},{2}}")
	xi := make([][]int, 3)
	xi[0] = []int{1, 2, 3}
	xi[1] = []int{1}
	xi[2] = []int{2}
	...
	expxi[1] = []int{1}
	expxi[2] = []int{2}
	alldifferent_Among_test(t, xi, expxi, true)
}

func Test_AlldifferentAmongb(t *testing.T) {
	setup()
	defer teardown()
	log("Alldifferent_Among: Xi:{{1,2},{1,2},{3,4},{3}}")
	xi := make([][]int, 4)
	xi[0] = []int{1, 2}
	xi[1] = []int{1, 2}
	xi[2] = []int{3, 4}
...
func Test_AlldifferentAmongc(t *testing.T) {
	setup()
	defer teardown()
	log("Alldifferent_Among: Xi:{{1,2,3},{3,4},{5,6}}")
	xi := make([][]int, 3)
	xi[0] = []int{1, 2, 3}
	xi[1] = []int{3, 4}
	xi[2] = []int{5, 6}
