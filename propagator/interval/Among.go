...
func (this *Among) Start(store *core.Store) {
	...
	for changeEntry := range this.inCh {
		...
		case this.n:
			nDom := this.varidToDomainMap[this.n]
			nDom.Removes(changeEntry.GetValues())
			...
			if changeValues[this.minN] || changeValues[this.maxN] {
				this.minN, this.maxN = nDom.GetMinAndMax()
				this.NinXiout(evt)
			}
			break
		default:
			...
			//from this map
			xDom := this.varidToDomainMap[var_id]
			if xDom != nil {
				...
				if xDom.IsSubset(this.k) {
					...
					this.lb += 1
					delete(this.varidToDomainMap, var_id)
					this.NinXiout(evt)
...
//the method also propagates N initially and removes old lbs and ubs from its domain
func (this *Among) removeIrrelevantDomains(evt *core.ChangeEvent) {
	...
	this.lb = 0
	this.ub = len(this.xi)
	n_Domain := this.varidToDomainMap[this.n]
	...
	for _, varid := range valuesToRemove {
		delete(this.varidToDomainMap, varid)
	}
}
...
func (this *Among) NinXiout(evt *core.ChangeEvent) {
	...
	addLb := 0
	var chEntry *core.ChangeEntry = nil
	//if only one element exists in D(N) which has the same value as the lower bound
	if this.lb == this.maxN {
		for _, x_id := range this.xi {
			...
			if domain != nil {
				...
				if !domain.IsSubset(this.k) && (!domain.Intersection(this.k).IsEmpty()) {
					if chEntry == nil {
...
func (this *Among) initialConsistencyCheck(evt *core.ChangeEvent) {
	this.minN, this.maxN = this.varidToDomainMap[this.n].GetMinAndMax()
...
func (this *Among) GetDomains() []core.Domain {
	domains := make([]core.Domain, len(this.varidToDomainMap))
	for _, var_id := range this.xi {
		domains = append(domains, this.varidToDomainMap[var_id])
	}
	return domains
}
