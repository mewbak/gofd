...
// C1XmultC2YeqC3ZBounds represents the constraint C1*X * C2*Y = C3*Z,
...
func (this *C1XmultC2YeqC3ZBounds) Start(store *core.Store) {
		...
		for x_val := range x_Domain.Values_asMap() {
			if x_val != 0 {
				if (this.c3*z_val)%(this.c1*x_val) == 0 &&
					y_Domain.Contains(((this.c3*z_val)/(this.c1*x_val))/this.c2) {
					match = true
					break
				}
			} else {
...
func CreateGXmultYeqZ(x core.VarId, y core.VarId, z core.VarId) *C1XmultC2YeqC3ZBounds {
	if core.GetLogger().DoDebug() {
		core.GetLogger().Dln("CreateXmultYeqC-propagator")
	}
	prop := new(C1XmultC2YeqC3ZBounds)
...
func (this *C1XmultC2YeqC3ZBounds) String() string {
	return fmt.Sprintf("PROP_%d %d*%s*%d*%s=%d*%s",
		...
		this.c3, core.GetNameRegistry().GetName(this.z))
}
